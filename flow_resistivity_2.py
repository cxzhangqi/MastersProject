...
def numerical_distance(frequency, eff_flow_resistivity, source_position, receiver_position, k1, Z1, R_p):
    ...
    second_term = (k1 * r2)**0.5

    third_term = np.cos(np.pi / 2 - reflection_angle) + Z2
...
def ground_wave_function(frequency, eff_flow_resistivity, source_position, receiver_position, k1, Z1, R_p):
    ...
    return 1 - 1j * np.pi**0.5 * w * np.e**(-w**2) * erfc(1j * np.sqrt(w))
...
def pressure_at_receiver(frequency, eff_flow_resistivity, source_position, receiver_position, temp = 20, RH = 80):
    ...
    r1 = np.linalg.norm(source_position[:-1] - receiver_position[:-1])
    r2, reflection_angle = calculate_reflection(source_position, receiver_position)
...
logy = 20*np.log10(pressure_at_receiver(frequency, sigma, a, m))
print(y.shape)

plt.figure(1)
plt.plot(frequency, np.abs(logy))
plt.xscale("log")
plt.show()
